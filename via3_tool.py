...
class Via3Json(object):
    ...
    def dumpAttributes(self, attributes_dict):
        ...
        for i, attribute_key in enumerate(attributes_dict, 1):
            ...
            _VIA_ATTRIBUTE_ANCHOR = {
                ...
                # File region composed of multiple disconnected regions
                'FILE1_Z0_XYN': '__FUTURE__',
                # Time marker in video or audio (e.g tongue clicks, speaker diarisation)
                'FILE1_Z1_XY0': '__FUTURE__',
                'FILE1_Z1_XY1': 'Spatial Region in a Video Frame (e.g. bounding box of an object)',
                # A video frame region composed of multiple disconnected regions
                'FILE1_Z1_XYN': '__FUTURE__',
                ...
                # A temporal segment with regions defined for start and end frames
                'FILE1_Z2_XYN': '__FUTURE__',
                ...
                # one region defined for each file (e.g. an object in multiple views)
                'FILEN_Z0_XYN': '__FUTURE__',
                ...
                # one timestamp for each video or audio file (e.g. for alignment)
                'FILEN_ZN_XY0': '__FUTURE__',
            ...
            assert attribute.get('anchor_id', False) and (
                attribute.get('anchor_id') in _VIA_ATTRIBUTE_ANCHOR)
    ...
    def dumpMetedatas(self, metadatas_dict):
        ...
        for i, metadatas_key in enumerate(metadatas_dict, 1):
            ...
            assert metadata_dict.get(
                'z', False) or metadata_dict.get('xy', False)
            assert metadata_dict.get('av', False) and isinstance(
                metadata_dict['av'], (dict,))

            if not metadata_dict.get('flg', False):
                metadata_dict['flg'] = 0

            # file location { LOCAL:1, URIHTTP:2, URIFILE:3, INLINE:4 }
            if not metadata_dict.get('z', False):
                metadata_dict['z'] = []

            # file location { LOCAL:1, URIHTTP:2, URIFILE:3, INLINE:4 }
            if not metadata_dict.get('xy', False):
                metadata_dict['xy'] = []
    ...
    def dumpViews(self, views_dict):
        ...
        for i, views_key in enumerate(views_dict, 1):
            ...
            assert view_dict.get('fid_list', False) and isinstance(
                view_dict.get('fid_list'), (list,))
